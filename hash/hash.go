...
// Lens reshare / activation / pairwise routine.
//
// Two profiles are shipped:
//
//   - Lens-SHA3   — the production profile. Built on cSHAKE256, KMAC256,
//     and TupleHash256 from FIPS 202 / NIST SP 800-185. KATs in the
//     reshare oracle are emitted under this profile.
//
//   - Lens-BLAKE3 — the legacy / non-normative profile. Preserved so
...
// HashSuite is the canonical hashing surface every Lens reshare,
...
type HashSuite interface {
	// ID returns the profile identifier, e.g. "Lens-SHA3" or
	// "Lens-BLAKE3". Bound into transcripts so two profiles can
	...
	// challenge seed before expansion to a binding factor).
...
var defaultSuite HashSuite = NewLensSHA3()
