...
// implementation_version, variant) tuple. The transcript hash is the
// single point of equality the new committee uses when it threshold-
// signs the activation message under the unchanged GroupKey: the chain
// accepts the new epoch only when the activation signature verifies
// against this hash.
//
// Domain separation. Every input field is unambiguously length-prefixed
// (TupleHash framing under Lens-SHA3, hand-rolled length prefixes under
// the legacy Lens-BLAKE3 suite). The customization tag is
// "LENS-TRANSCRIPT-v1" — distinct from any DKG transcript tag and from
// any Lens Sign transcript tag.
...
// (Lens-SHA3); pass hash.NewLensBLAKE3() to reproduce legacy bytes.
...
func ValidatorSetHash(publicKeys [][]byte, suite hash.HashSuite) [32]byte {
	...
	parts = append(parts, []byte("lens.reshare.validator-set.v1"))
	var nBuf [4]byte
